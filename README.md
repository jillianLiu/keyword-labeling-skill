...
1. 先准备原始关键词表，确认关键词列和原始数据都保留完整。
2. 运行 intake，把原始表整理成 fact table，并生成稳定的 keyword ID。
3. 检查 intake 行数，确认没有漏词、原始词没有被改写。
4. 把 fact table 按每批 200 个词切成多个 batch。
5. 让 annotation subagent 逐批做语义打标。语义判断必须独立完成，不能直接照抄 intake 提示。
6. 对含义不清、品牌/平台/竞品、风险词或 QA 指出的词做网络搜索核验。
7. 每批标注完先跑结构校验，确认没有漏行、重复 ID、未知 ID 或字段错误。
8. 让独立 QA subagent 审核该批次。QA 不直接修改 annotation 文件。
9. QA 失败时，少量行问题走 targeted repair；系统性问题走 full rerun。
10. 修复或重跑后重新结构校验，再跑 final QA。
11. 只把 final QA 通过的批次放进 merge-ready 目录。
12. 合并后必须跑覆盖检查，确认没有遗漏、重复或未知关键词 ID。
13. 最后导出中文审阅表和可筛选 `.xlsx` 工作簿，交给人工审核。
